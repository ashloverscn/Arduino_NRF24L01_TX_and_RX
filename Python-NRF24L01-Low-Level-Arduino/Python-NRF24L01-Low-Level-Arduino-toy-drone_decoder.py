...
# ═══════════════════════════════════════════════════════════════════════
# NRF24L01+ SPI Protocol Constants (for Arduino SPI Proxy)
# ═══════════════════════════════════════════════════════════════════════
...
class NRF24Controller:
    ...
    This backend translates high-level NRF24 commands into raw SPI operations
    ...
    def _spi_transfer(self, data: int) -> int:
    ...
    def _spi_burst(self, data_bytes: List[int]) -> List[int]:
    ...
    def _read_register(self, reg_addr: int) -> int:
        ...
        self._csn_low()
        self._spi_transfer(R_REGISTER | (reg_addr & 0x1F))
    ...
    def _read_multi_register(self, reg_addr: int, length: int) -> List[int]:
    ...
    def _write_multi_register(self, reg_addr: int, data_bytes: List[int]):
    ...
    def _read_rx_payload(self, length: int) -> List[int]:
        self._csn_low()
    ...
    def _write_tx_payload(self, data_bytes: List[int]):
        self._csn_low()
    ...
    # ═══════════════════════════════════════════════════════════════════
    # HIGH-LEVEL COMMAND TRANSLATION (called by PromiscuousScanner/GUI)
    # ═══════════════════════════════════════════════════════════════════
    ...
    def send(self, cmd: str, wait: bool = False, timeout: float = 0.5) -> Optional[str]:
        ...
        try:
            ...
            # ─── Set Mode (RX/TX/Standby/PowerDown) ─────────────────────
            elif op == 'X':
            ...
            # ─── Multi-byte Write ───────────────────────────────────────
                ...
                return f"Wrote {' '.join(f'{b:02X}' for b in data)} to 0x{addr:02X}"
